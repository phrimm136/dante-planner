...
resource "aws_vpc" "this" {
  ...
  tags                 = merge(var.tags, { Name = "${var.name_prefix}-oregon" })
}

resource "aws_internet_gateway" "this" {
  vpc_id = aws_vpc.this.id
  tags   = merge(var.tags, { Name = "${var.name_prefix}-oregon-igw" })
}
...
resource "aws_subnet" "public" {
  ...
  tags                    = merge(var.tags, { Name = "${var.name_prefix}-oregon-public-${var.availability_zones[count.index]}" })
}
...
resource "aws_security_group" "cluster" {
  name        = "${var.name_prefix}-oregon-cluster"
  description = "k3s intra-cluster traffic (self) + egress"
  vpc_id      = aws_vpc.this.id
  tags        = merge(var.tags, { Name = "${var.name_prefix}-oregon-cluster" })
}
...
resource "aws_security_group" "ingress" {
  ...
  vpc_id      = aws_vpc.this.id
  tags        = merge(var.tags, { Name = "${var.name_prefix}-oregon-ingress" })
}
