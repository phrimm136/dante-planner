# Install the bundled-manifest converger: k3s re-applies its bundled manifests
# (CoreDNS, metrics-server) on every server start, reverting the pod-template
# scrape annotations and the metrics-server placement, so a k3s-start-triggered
# oneshot re-converges them. Consumed by cp.sh.tftpl (new instances) and the
# State Manager association (live instances) — idempotent.
...
until $KUBECTL -n kube-system patch deployment metrics-server --type merge -p '{"spec":{"template":{"spec":{"nodeSelector":{"role":"data"}}}}}'; do sleep 10; done
EOF
