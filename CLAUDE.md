...
11. **NEVER cd** - Use in-command directory flags (`--cwd`, `-p`, `-C`, etc.) to target subdirectories. `cd` confuses the session's working directory and compound commands with `cd` get blocked by security evaluation. Tests/builds MUST target the project root explicitly — `yarn --cwd frontend …`, `/home/user/github/LimbusPlanner/backend/gradlew -p backend …` — a bare `vitest`/`tsc`/`gradlew` at the repo root is hook-blocked.
