...
# Hex transit: the SQL file binds @pw from @pw_hex, so no quoting rule of the
# secret's own bytes can reach the parser.
PW_HEX=$(printf '%s' "$PW" | od -An -v -tx1 | tr -d ' \n')

{ printf "SET @pw_hex = '%s';\n" "$PW_HEX"; cat "$OPS_DIR/rds-readonly-user.sql"; } |
