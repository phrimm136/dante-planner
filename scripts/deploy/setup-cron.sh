...
sudo touch /var/log/danteplanner-backup.log /var/log/cloudflare-ip-update.log /var/log/danteplanner-cw-export.log /var/log/danteplanner-query-stats.log
sudo chown ec2-user:ec2-user /var/log/danteplanner-backup.log /var/log/cloudflare-ip-update.log /var/log/danteplanner-cw-export.log /var/log/danteplanner-query-stats.log
...
# MySQL query-digest latency: every 5 minutes (matches dashboard period 300)
QUERY_STATS_CRON="*/5 * * * * /opt/danteplanner/scripts/ops/emit-mysql-query-stats.sh >> /var/log/danteplanner-query-stats.log 2>&1"
...
{ crontab -l 2>/dev/null | grep -v -e "aws-backup.sh" -e "update-cloudflare-ips.sh" -e "export-cloudwatch-logs.sh" -e "emit-mysql-query-stats.sh" || true; echo "$BACKUP_CRON"; echo "$CF_IP_CRON"; echo "$CW_EXPORT_CRON"; echo "$QUERY_STATS_CRON"; } | crontab -
