...
export const StatusViewer: React.FC<StatusViewerProps> = ({ deckState }) => {
  ...
  const deployedSinnerCodes = (() => {
    return deckState.deploymentOrder.map((index) => String(index + 1))
  })()

  // Split into deployed-only (first 7) and all sinners for keyword EA calculation
  const deployedOnlySinnerCodes = (() => {
  ...
  const affinityCounts: AffinityCount[] = (() => {
  ...
  const keywordCounts: (KeywordCount & { deployedCount: number; allCount: number })[] = (() => {
      ...
      <div className="flex flex-wrap gap-2 min-h-7 items-center">
        {keywordCounts.map(({ keyword, count, deployedCount, allCount }) => {
          ...
          const textColorClass =
            deployedCount >= 5 ? 'text-yellow-400' : allCount >= 5 ? 'text-cyan-400' : ''
