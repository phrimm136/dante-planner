...
describe('AnnouncementDialog', () => {
  it('UT8: opens in list view showing all announcements', () => {
    render(
      <AnnouncementDialog announcements={mockAnnouncements} open={true} onOpenChange={vi.fn()} />,
    )
    ...
    expect(
      screen.queryByText((_, el) => el?.textContent === 'First body text\nWith newlines'),
  ...
  it('UT9: clicking a row transitions to detail view', async () => {
    ...
    expect(
      screen.getByText((_, el) => el?.textContent === 'First body text\nWith newlines'),
    ).toBeInTheDocument()
  ...
  it('UT11: close and reopen resets to list view', async () => {
    ...
    rerender(
      <AnnouncementDialog announcements={mockAnnouncements} open={false} onOpenChange={vi.fn()} />,
    )
  ...
  it('UT12: empty announcements list shows noAnnouncements string', () => {
    render(<AnnouncementDialog announcements={[]} open={true} onOpenChange={vi.fn()} />)
