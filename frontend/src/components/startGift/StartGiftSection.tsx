...
interface StartGiftSectionProps {
  mdVersion: MDVersion
 ...
 */
function calculateMaxGiftSelection(buffs: StartBuff[] | undefined, selectedIds: Set<number>): number {
  if (!buffs) return 1
...
/**
 * Section for selecting start EGO gifts in the Mirror Dungeon planner
 ...
 * - Resets gift selection when EA (selection count) changes
 */
export function StartGiftSection({
  mdVersion,
  selectedBuffIds,
  selectedKeyword,
  selectedGiftIds,
  ...
  // Reset gift selection when EA changes
  useEffect(() => {
    if (selectedGiftIds.size > maxSelectable) {
      // Keep only up to maxSelectable gifts (first N selected)
