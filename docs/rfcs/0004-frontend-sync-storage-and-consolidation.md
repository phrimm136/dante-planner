...
| `CatalogSlice<TItem>` with an asymmetric ego extension | — | SUPERSEDED: `FilteredEntityGrid` already collapsed the five lists (45-61 lines each); `maxThreadspin` is read above and below the lists, never in them; four sorters exist, not one asymmetric; the varying parts close over hook results and cannot live in a static descriptor |
| `ThemePackDescriptor` replaces loose theme-pack params | — | SUPERSEDED: `FloorThemeSelection` already is the descriptor and is threaded end to end; the residual loose params guard nullability the bundle would reintroduce |
...
| Remove `keepPrevious` everywhere; rewrite the rule to name skeletons + Deferred hooks | `lib/queryOptions.ts:9,12,50`; the 6 `{keepPrevious:true}` callers; `src/pages/CLAUDE.md:15` | Under Suspense the boundary owns loading; on the non-suspending Deferred hooks the removal is a deliberate UX choice — a language switch falls to the empty/skeleton state instead of holding stale-language names (user ruling) |
