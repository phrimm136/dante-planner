...
>
> **Last Updated:** 2026-03-31 (cc77884f: add plan search by content items)
...
| **Planner List** | `routes/PlannerMDPage.tsx` (personal), `routes/PlannerMDGesellschaftPage.tsx` (community, ErrorBoundary) | `hooks/useMDUserPlannersData.ts`, `hooks/useMDGesellschaftData.ts`, `hooks/useMDUserFilters.ts`, `hooks/useMDGesellschaftFilters.ts`, `hooks/usePlannerSearchFilters.ts` (URL param sync for content filters), `types/MDPlannerListTypes.ts`, `types/PlannerSearchTypes.ts`, `components/plannerList/MDPlannerNavButtons.tsx`, `components/plannerList/MDPlannerToolbar.tsx`, `components/plannerList/PlannerFilterPane.tsx` (collapsible content filter with progressive loading), `components/plannerList/PersonalPlannerCard.tsx`, `components/plannerList/PublishedPlannerCard.tsx`, `components/plannerList/PlannerCardContextMenu.tsx` (independent vote/fork mutation states), `lib/plannerContentExtractors.ts` (entity ID extraction for local filtering), `hooks/usePlannerKeywordsI18n.ts`, `components/home/CommunityPlansErrorFallback.tsx` (shared error fallback) |
| **Planner Detail (Viewer)** | `routes/PlannerMDDetailPage.tsx` (personal), `routes/PlannerMDGesellschaftDetailPage.tsx` (community) | `hooks/usePublishedPlannerQuery.ts`, `hooks/usePlannerSubscription.ts`, `hooks/usePlannerReport.ts`, `hooks/usePlannerDelete.ts`, `hooks/useProgressiveReveal.ts` (staggered section reveal), `components/plannerViewer/PlannerViewer.tsx` (CSS visibility toggle for instant mode switching), `components/plannerViewer/PlannerDetailHeader.tsx` (published/personal variants; Apply Latest Mirror syncs to server when published or sync-on), `components/plannerViewer/ApplyLatestMirrorDialog.tsx`, `components/plannerViewer/PlannerDetailFooter.tsx` (independent vote/fork mutation states), `components/plannerViewer/CopyUrlButton.tsx`, `components/plannerViewer/DeleteConfirmDialog.tsx`, `components/plannerViewer/GuideModeViewer.tsx`, `components/plannerViewer/TrackerModeViewer.tsx` (side-by-side gift/theme layout), `components/plannerViewer/ComprehensiveGiftGridTracker.tsx` (keyword filter, search bar), `components/plannerViewer/HorizontalThemePackGallery.tsx` (floor indicators, ThemePackViewer) |
...
|--------|------------|------------------|
| **Authentication** | `controller/AuthController.java` (removed POST /refresh, /me returns 200+null for guests) | `service/token/JwtTokenService.java` (RS256 signing + AES-256-GCM payload encryption), `config/JwtProperties.java` (PEM key loading), `service/oauth/*`, `security/JwtAuthenticationFilter.java` (auto-refresh with token rotation), `security/CustomAuthenticationEntryPoint.java` (generic 401 responses), `service/token/TokenBlacklistService.java` (refresh token rotation) |
...
| **Planner Publishing** | `service/PlannerService.java` (togglePublish, castVote) | `entity/PlannerVote.java`, `entity/VoteType.java`, `repository/PlannerVoteRepository.java`, `repository/PlannerRepository.java` (atomic increments without clearAutomatically, JpaSpecificationExecutor), `dto/planner/PublicPlannerResponse.java`, `dto/planner/VoteRequest.java`, `converter/KeywordSetConverter.java` |
| **Planner Content Search** | `service/PlannerIndexService.java` (reindex/deleteIndex on publish), `specification/PlannerSpecifications.java` (composable query filters) | `entity/PlannerContentIndex.java`, `entity/PlannerContentIndexId.java`, `entity/ContentEntityType.java`, `repository/PlannerContentIndexRepository.java`, `controller/PlannerController.java` (structured query params: keyword, identity, ego, gift, themePack) |
...
| **Observability / MDC** | N/A | `security/MdcLoggingFilter.java` (injects requestId, userId, method, path into SLF4J MDC; runs after JwtAuthenticationFilter; CRLF-sanitized path; FilterRegistrationBean prevents double-registration), `resources/logback-spring.xml` (prod: INFO+ LogstashEncoder JSON → stdout → awslogs → `/ecs/danteplanner/backend`; exported daily to S3 by `export-cloudwatch-logs.sh`; dev: colored console only), `nginx/nginx.conf` (JSON access log format via `log_format json_combined escape=json`), `scripts/ops/setup-cloudwatch-alarms.sh` (metric filters, alarms, 15 Insights queries for backend/nginx/mysql, CloudWatch dashboard), `scripts/ops/export-cloudwatch-logs.sh` (daily CW → S3 export for backend, nginx, mysql), `scripts/ops/cloudwatch-agent-config.json` (CW Agent: cron op logs only; backend logs go via awslogs) |
...
| **Auth Tokens** | HttpOnly cookies (access + refresh). **Encryption**: RS256 asymmetric signing + AES-256-GCM payload encryption (custom claims encrypted, sub/iat/exp readable for JJWT validation). **Backend auto-refresh**: Filter transparently renews expired tokens with rotation (old refresh token blacklisted). **Security**: Generic 401 responses (no error codes exposed to prevent fingerprinting). **/me endpoint**: Public, returns 200+null for guests (no 401 console errors). **Frontend**: Simplified - no refresh logic, backend handles all token renewal | `service/token/JwtTokenService.java` (ThreadLocal SecureRandom), `config/JwtProperties.java` (RSA-2048 PEM loading), `security/JwtAuthenticationFilter.java` (attemptAutoRefresh), `security/CustomAuthenticationEntryPoint.java`, `service/token/TokenBlacklistService.java` |
