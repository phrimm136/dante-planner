...
Companion to `requirements.md`. Implementation-grade mechanics for the stateless refactor + causal-consistency stack, transcribed from the k8s-phase2/phase3 design sessions and the 2026-07-05/06 architecture debate. The full Lua rotation trace (normal chain A→B→C, theft-via-replay) was worked through in session `k8s-phase3`; the contract below is binding, the trace is reference.
...
- Toxiproxy profiles: `wan` (130ms latency app→primary), `partition` (timeout toxic = peering loss). Suites: `ReplicaLagIT`, `CausalGateIT`, `DegradationIT`, `BulkheadIT` per requirements.md Test Plan.
