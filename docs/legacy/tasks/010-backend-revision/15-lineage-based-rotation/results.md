...
**Docs:** `docs/tasks/010-backend-revision/15-lineage-based-rotation/{plan,status,review-findings,announcement-draft,results}.md`
...
- Manual: not run (staging-only steps in requirements.md §Verification remain for rollout)
...
- **Next steps**: (1) user decides on `review-findings.md` items #3/#4/#5; (2) commit (note `static` submodule commits separately; no Co-Authored-By line); (3) optional `/review` or `/ultrareview` before PR; (4) staging rollout per requirements.md §Rollout.
