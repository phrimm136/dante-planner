...
- **Next steps**: (1) optionally add the recommended BE wire-key MockMvc test (`$.isBookmarked`/`$.isSubscribed`); (2) commit via `git commit -- backend/ scripts/ docs/32-be-pattern-consolidation/` (message drafted, in review-guide history); (3) work the recorded follow-ups (shared-sink relocation, ModerationAction @Index, V045 index drop, InternalController/events dedup).
- **Blockers**: none.
