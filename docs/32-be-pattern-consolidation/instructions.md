...
BE-only. Frontend refactoring and repo-root hygiene are owned by `docs/31-fe-structure-refactor/` (a parallel session); this task must not touch `frontend/` or root-level files owned by that spec.
...
- `docs/31-fe-structure-refactor/instructions.md` (parallel-session ownership boundary + pattern source)
