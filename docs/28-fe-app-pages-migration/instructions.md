...
- **Slice directory names are singular nouns matching the route** — `pages/keyword/` (route `/keyword`, matches `docs/20-keyword` and existing `identity`/`ego` singular convention), `pages/moderator/`, `pages/home/`, `pages/settings/`, `pages/legal/`. (evidence: existing `pages/<noun>/` dirs are singular)
